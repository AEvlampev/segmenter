...
from langprocessor.segmenter.morph_analyzer import get_tags, has_same_tense, is_geographical_object,\
    has_same_gender, is_plur_number
import langprocessor.segmenter.constituency_tree_builder.lists
from langprocessor.segmenter.constituency_tree_builder.utils import find_conjunction_parts_between, tokens_list,\
...
def is_punct(dtree):
    return dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._endpunct_pos
...
def is_verb_predicate(dtree):
    return dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._verb_predicate_pos and dtree["text"] is not None
...
def is_nominative_predicate(dtree):
    ...
    if is_main_part_of_compound_nominative_predicate(dtree):
        return True
    return dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._nominative_predicate_pos and \
                              any(map(lambda x: (x["pos"], x["deprel"]) in langprocessor.segmenter.constituency_tree_builder.lists._nominative_subject_pos_deprels,
                                      not_included_children(dtree)))


def is_verb_subject(dtree):
    has_pos = dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._verb_predicate_pos
    is_infinitive = dtree["text"] == dtree["lemma"]
    has_subject = any(map(lambda x: (x["pos"], x["deprel"]) in langprocessor.segmenter.constituency_tree_builder.lists._nominative_subject_pos_deprels,
                          not_included_children(dtree)))
...
def is_compound_part(dtree, parent):
    if is_divided_subordinative(dtree, parent):
        return False
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._compound_part_deprels
    has_subject = False
...
def is_divided_subordinative(dtree, main_dtree=None):
    ...
    if is_subordinated_direct_speech(dtree, main_dtree):
        return False
    if any(map(lambda x: ' '.join(x) == get_full_text(tokens_list(dtree)), langprocessor.segmenter.constituency_tree_builder.lists._sustainable_introductions)):
        return False
    ...
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._divided_subordinative_deprels
    if dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._compound_part_deprels or has_deprel:
        conjunction_parts = find_conjunction_parts_between(dtree, main_dtree)
        conjunction_text = get_full_text(conjunction_parts)
        if conjunction_text in langprocessor.segmenter.constituency_tree_builder.lists._conjunction_types and langprocessor.segmenter.constituency_tree_builder.lists._conjunction_types[conjunction_text] == "subordinative" \
                or conjunction_text == "—":
            has_subordinative_conj = True
    if main_dtree is not None and (dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._compound_part_deprels or has_deprel):
    ...
    for subject_candidate in not_included_children(dtree):
        if (subject_candidate["pos"], subject_candidate["deprel"]) in langprocessor.segmenter.constituency_tree_builder.lists._nominative_subject_pos_deprels:
            has_subject = True
            break
...
def is_subordinative(dtree, parent):
    ...
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._subordinative_deprels
...
def has_subordinative_conjunction(dtree, only_not_included=True):
    ...
    for conjunction_candidate in children:
        if conjunction_candidate["lemma"] in langprocessor.segmenter.constituency_tree_builder.lists._subordinative_conjunction_lemmas \
...
def is_adverbial(dtree, parent):
    if dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._particle_pos:
        return False
    if parent["pos"] == "ADV" and dtree["lemma"] in langprocessor.segmenter.constituency_tree_builder.lists._particle_by_type:
        return False
    return dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._adverbial_deprels or \
...
def is_adverbial_specific_preposition(dtree):
    if get_full_text(tokens_list(dtree)).lower() in langprocessor.segmenter.constituency_tree_builder.lists._adverbial_specific_preposition_lemmas:
        return True
    return False
...
def is_adverbial_specific_nominative(dtree, parent):
    ...
    if dtree["text"] == "%" and not_included_children(dtree) and not_included_children(dtree, natural_order=True)[0]["lemma"] == "на":
        return True
    has_listed_lemma = dtree["lemma"].lower() in (langprocessor.segmenter.constituency_tree_builder.lists._adverbial_specific_nominatives_lemmas | langprocessor.segmenter.constituency_tree_builder.lists._months_names)
    ...
    if is_verb_predicate(parent) or is_aux_part_of_compound_nominative_predicate(parent):
        if dtree["deprel"] == "nummod":
            for child in not_included_children(dtree):
                if child["lemma"].lower() in langprocessor.segmenter.constituency_tree_builder.lists._months_names:
                    return True
...
def is_indirect_object(dtree, parent):
    ...
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._indirect_object_deprels or \
...
def is_verbal_noun(dtree):
    return dtree["lemma"] in langprocessor.segmenter.constituency_tree_builder.lists._verbal_nouns
...
def is_definition(dtree, parent):
    ...
    if parent is not None and is_compound_name_proper_noun_part(dtree, parent):
        return False
    if dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._definition_deprels:
        return True
    tags = get_tags(dtree["text"], dtree["lemma"], dtree["pos"])
    if tags["pos"] in {"PRTF"}:
        return True
...
def is_direct_object(dtree, parent):
    if is_enquoted(dtree) and is_verb_predicate(dtree):
        return True
    if dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._direct_object_deprels:
        return True
...
def is_flat_object_part(dtree, parent):
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._flat_object_part_deprels
    is_atomic_part = is_atomic_nominative_part(dtree, parent)
...
def is_aux_verb_text(dtree):
    if dtree["lemma"].lower() in langprocessor.segmenter.constituency_tree_builder.lists._aux_verb_specific_lemmas:
        return True
    if dtree["text"] == "смела":
        return True
    return False
...
def is_main_verb(dtree):
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._main_verb_deprels
    return has_deprel
...
def is_main_part_of_compound_nominative_predicate(dtree, parent=None):
    ...
    tags = get_tags(dtree["text"], dtree["lemma"], dtree["pos"])
    has_pos = dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._nominative_pos or tags["pos"] == "PRTS"
...
def is_aux_part_of_compound_nominative_predicate(dtree):
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._aux_verb_for_nominative_deprels
...
def find_introduction(core_dtree):
    ...
    if is_enquoted(candidates[0]) or is_enclosed_in_brackets(candidates[0]):
        return None
    if candidates[0]["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._introduction_deprels \
        ...
        if not (len(conjunctions_between) == 1 and conjunctions_between[0]["text"] == ":"):
            return candidates[0]
    if candidates[0]["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._divided_subordinative_deprels and len(not_included_children(candidates[0])) > 0:
        ...
        has_lemma = possible_conjunction["lemma"].lower() in langprocessor.segmenter.constituency_tree_builder.lists._introduction_conjunction_specific_lemmas
...
def is_enclosed_in_brackets(dtree):
    ...
    if len(lemmas) < 2:
        return False
    return lemmas[0] in langprocessor.segmenter.constituency_tree_builder.lists._opening_brackets and lemmas[-1] in langprocessor.segmenter.constituency_tree_builder.lists._closing_brackets
...
def is_nominative_subject(dtree, predicate=None):
    if predicate is None:
        return dtree["pos"] in {pos for pos, deprel in langprocessor.segmenter.constituency_tree_builder.lists._nominative_subject_pos_deprels}
    return (dtree["pos"], dtree["deprel"]) in langprocessor.segmenter.constituency_tree_builder.lists._nominative_subject_pos_deprels
...
def find_sustainable_introduction(dtree):
    ...
    for sustainable_introduction_candidate in langprocessor.segmenter.constituency_tree_builder.lists._sustainable_introductions:
...
def is_particle(dtree):
    if dtree["pos"] in langprocessor.segmenter.constituency_tree_builder.lists._particle_pos or dtree["text"].lower() in {"бы", "даже", "уже"}:
        return True
    return False


def is_homogeneous_predicates_part(dtree, parent):
    has_deprel = dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._homogeneous_part_deprels
...
def is_preposition(dtree):
    if dtree["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._preposition_deprels:
        return True
    if dtree["lemma"].lower() in langprocessor.segmenter.constituency_tree_builder.lists._preposition_specific_lemmas:
        return True
    return False
...
def has_coordinative_conjunction(dtree):
    for coordination_candidate in not_included_children(dtree):
        if coordination_candidate["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._conjunction_deprels \
                and langprocessor.segmenter.constituency_tree_builder.lists._conjunction_types[coordination_candidate["text"]] == "coordinative":
            return True
    return False
...
def is_enquoted(dtree):
    ...
    if len(children) < 2:
        return False
    return children[0]["text"] in langprocessor.segmenter.constituency_tree_builder.lists._opening_quotes and children[-1]["text"] in langprocessor.segmenter.constituency_tree_builder.lists._closing_quotes
