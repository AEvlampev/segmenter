...
from langprocessor.segmenter.constituency_tree_builder.checks import is_predicate, is_verb_predicate, is_verb_subject, \
    is_compound_part, \
...
def create_core(dtree):
    ...
    if len(compound_parts_roots) > 1:
        ...
        for ix, part in enumerate(compound_parts_roots):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-sentence"] = create_core(part)
        return result
...
def create_introduction(dtree, parent=None):
    ...
    if len(not_included_children(dtree)) != 0:
        ...
        for ix, part in enumerate(parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-introduction-part"] = create_introduction(part)
        return result
    dtree["~included"] = True
    return {
        "_type": "introduction",
...
def create_verb_subject(dtree):
    ...
    for direct_object_candidate in not_included_children(dtree):
        if direct_object_candidate["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._direct_object_deprels:
...
def create_subject(dtree):
    ...
    for homogeneous_candidate in not_included_children(dtree):
        if homogeneous_candidate["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._homogeneous_part_deprels:
    ...
    if len(homogeneous_parts) > 1:
        ...
        for ix, part in enumerate(homogeneous_parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-subject"] = create_subject(part)
        return result
...
def create_predicate(dtree, parent=None):
    ...
    if len(homogeneous_parts) > 1:
        ...
        if conjunction_parts:
            ...
            result["joined-by"] = conjunction
        for ix, part in enumerate(homogeneous_parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-predicate"] = create_predicate(part, dtree)
        return result
    ...
    if is_main_part_of_compound_verb_predicate(dtree):
        for aux_part_candidate in not_included_children(dtree):
            if aux_part_candidate["lemma"].lower() in langprocessor.segmenter.constituency_tree_builder.lists._aux_verb_specific_lemmas:
                aux_part = aux_part_candidate
...
def create_main_nominative(dtree):
    ...
    if len(homogeneous_parts) > 1:
        ...
        if conjunction_parts:
            ...
            result["joined-by"] = conjunction
        for ix, part in enumerate(homogeneous_parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-main-nominative"] = create_main_nominative(part)
        return result
    ...
    for preposition_candidate in not_included_children(dtree):
        if preposition_candidate["deprel"] in langprocessor.segmenter.constituency_tree_builder.lists._preposition_deprels:
            preposition = preposition_candidate
            preposition["~included"] = True
            return {
                "_type": "main-nominative",
...
def create_object(dtree):
    ...
    if len(homogeneous_parts) > 1:
        ...
        if conjunction_parts:
            ...
            result["joined-by"] = conjunction
        for ix, part in enumerate(homogeneous_parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-object"] = create_object(part)
        return result
    if has_introduction(dtree):
        introduction = find_introduction(dtree)
        introduction["~included"] = True
...
def create_adverbial(dtree):
    ...
    if len(homogeneous_parts) > 1:
        ...
        if conjunction_parts:
            ...
            result["joined-by"] = conjunction
        for ix, part in enumerate(homogeneous_parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-adverbial"] = create_adverbial(part)
        return result
...
def create_definition(dtree):
    ...
    if len(homogeneous_parts) > 1:
        ...
        for ix, part in enumerate(homogeneous_parts):
            result[f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-definition"] = create_definition(part)
        conjunction_parts = find_conjunction_parts_between(*homogeneous_parts)
        if conjunction_parts:
            conjunction = create_conjunction(*conjunction_parts)
            result["joined-by"] = conjunction
...
def create_enclosing_quotes(dtree):
    ...
    for child in children:
        if child["text"] in langprocessor.segmenter.constituency_tree_builder.lists._opening_quotes:
            child["~included"] = True
            left = child
    for child in reversed(children):
        if child["text"] in langprocessor.segmenter.constituency_tree_builder.lists._closing_quotes:
            child["~included"] = True
            right = child
...
def create_enclosing_brackets(dtree):
    ...
    left, right = None, None
    for child in children:
        if child["text"] in langprocessor.segmenter.constituency_tree_builder.lists._opening_brackets:
            child["~included"] = True
            left = child
    for child in reversed(children):
        if child["text"] in langprocessor.segmenter.constituency_tree_builder.lists._closing_brackets:
            child["~included"] = True
            right = child
    assert left is not None and right is not None
    left["text"] = f"{left['text']} {right['text']}"
...
def create_subordinative(dtree, main_word):
    ...
    if conjunction_parts:
        ...
        if conjunction_text in langprocessor.segmenter.constituency_tree_builder.lists._definitive_subordinative_conjunctions:
            type_ = "definition"
        elif conjunction_text in langprocessor.segmenter.constituency_tree_builder.lists._direct_object_subordinative_conjunctions:
            type_ = "direct-object"
        elif conjunction_text in langprocessor.segmenter.constituency_tree_builder.lists._adverbial_subordinative_conjunctions:
            type_ = "adverbial"
        else:
...
def create_subordinated_direct_speech(dtree, main_word):
    ...
    if children[0]["text"] in langprocessor.segmenter.constituency_tree_builder.lists._opening_quotes:
        joinings.append(children[0])
    if children[-1]["text"] in langprocessor.segmenter.constituency_tree_builder.lists._closing_quotes:
...
def create_conjunction(*parts):
    ...
    return {
        "_type": f"{langprocessor.segmenter.constituency_tree_builder.lists._conjunction_types[text.lower()]}-conjunction",
        "_token": base_token
    }
...
def create_preposition(dtree):
    ...
    return {
        "_type": "preposition",
        **{f"{langprocessor.segmenter.constituency_tree_builder.lists._parts_names[ix]}-part": create_preposition(part) for ix, part in enumerate(parts)}
    }


def create_particle(dtree):
    text = dtree["lemma"].lower()
    type_ = langprocessor.segmenter.constituency_tree_builder.lists._particle_by_type[text]
    return {
        "_type": type_,
        "_token": dtree
    }
